...
public:
    ...
    ~KeyButton();
    bool GetStatus() const override { return status.load(); }
...
public:
    ...
    void RemoveButton(const KeyButton* key_button) {
        std::lock_guard<std::mutex> guard(mutex);
        list.remove_if([key_button](const KeyButtonPair& pair) { return pair.key_button == key_button; });
    }
...
KeyButton::~KeyButton() { button_list->RemoveButton(this); }

// Analog Button
class AnalogButton final : public Input::ButtonDevice {
public:
    explicit AnalogButton(std::shared_ptr<AnalogButtonList> button_list_, float threshold_, bool trigger_if_greater_)
    : button_list(button_list_), threshold(threshold_), trigger_if_greater(trigger_if_greater_) {}
    ~AnalogButton();
    bool GetStatus() const override {
        if (trigger_if_greater) return axis_val.load() > threshold;
...
public:
    ...
    void RemoveButton(const AnalogButton* key_button) {
        std::lock_guard<std::mutex> guard(mutex);
        list.remove_if([key_button](const AnalogButtonPair& pair) { return pair.key_button == key_button; });
    }
...
AnalogButton::~AnalogButton() { button_list->RemoveButton(this); }
...
public:
    ...
    ~Joystick();
    std::tuple<float, float> GetStatus() const override { return std::make_tuple(x_axis.load(), y_axis.load()); }
...
public:
    ...
    void RemoveButton(const Joystick* key_button) {
        std::lock_guard<std::mutex> guard(mutex);
        list.remove_if([key_button](const AnalogPair& pair) { return pair.key_button == key_button; });
    }
...
Joystick::~Joystick() { analog_list->RemoveButton(this); }

ButtonFactory::ButtonFactory()
: button_list{std::make_shared<ButtonList>()}, analog_button_list{std::make_shared<AnalogButtonList>()} {}
...
std::unique_ptr<Input::ButtonDevice> ButtonFactory::Create(const Common::ParamPackage& params) {
    if (params.Has("axis")) {
        ...
        bool trigger_if_greater = (direction_name == "+");
        std::unique_ptr<AnalogButton> analog_button = std::make_unique<AnalogButton>(analog_button_list, threshold, trigger_if_greater);
...
bool ButtonFactory::PressKey(int button_id) { return button_list->ChangeButtonStatus(button_id, true); }
bool ButtonFactory::ReleaseKey(int button_id) { return button_list->ChangeButtonStatus(button_id, false); }
bool ButtonFactory::AnalogButtonEvent(int axis_id, float axis_val) { return analog_button_list->ChangeAxisValue(axis_id, axis_val); }
...
bool AnalogFactory::MoveJoystick(int analog_id, float x, float y) { return analog_list->ChangeJoystickStatus(analog_id, x, y); }

std::unique_ptr<Input::MotionDevice> MotionFactory::Create(const Common::ParamPackage &params) { return nullptr; }
void MotionFactory::EnableSensors() {}
void MotionFactory::DisableSensors() {}

ButtonFactory* ButtonHandler() { return button.get(); }
AnalogFactory* AnalogHandler() { return analog.get(); }

std::string GenerateButtonParamPackage(int button) {
    Common::ParamPackage param{{"engine", "gamepad"}, {"code", std::to_string(button)}};
    return param.Serialize();
}

std::string GenerateAnalogParamPackage(int axis_id) {
    Common::ParamPackage param{{"engine", "gamepad"}, {"code", std::to_string(axis_id)}};
    return param.Serialize();
}
